...
capitalized by a fine-tuned multilingual punctuation model (see `PUNCT_MODEL_DIR`).
...
| `PUNCT_MODEL_DIR` | No | Fine-tuned punctuation/capitalization model (local directory or HF id). When set, transcribed text is punctuated and sentence-start capitalized. Blank → transcripts stay lowercase and unpunctuated. |
...
When a punctuation model is configured (`PUNCT_MODEL_DIR`), all transcription endpoints
restore punctuation and capitalization by default. Every transcription endpoint accepts
an optional **`punctuate`** field (`bool`, default `true`) — set it to `false` to get
the raw lowercase, unpunctuated transcript. It's a JSON field on `POST /transcribe` and a
multipart form field on the upload/OpenAI/segment endpoints (OpenAI SDK clients pass it via
`extra_body={"punctuate": false}`).
